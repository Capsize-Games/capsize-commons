...
| --- | --- | --- | --- |
| `capsize-commons` Python | `0.1.2` | Python `>=3.11` | `python/pyproject.toml`, PyPI package metadata |
| `@capsizellc/commons` | `0.1.1` | Node `>=22` | `typescript/package.json`, npm package metadata |
| C++ library | repository release | C++23, CMake `>=3.24` | `cpp/CMakeLists.txt` and tagged source release |
...
The current Python artifact `0.1.2` is published and verified. The checklist
does not claim trusted publishing: the tag-only OIDC workflow, package-owner
configuration, repeat-tag behavior, and recovery evidence remain gated by
`hq#23` and the unresolved `capsize-ci` authority in `hq#43`.
